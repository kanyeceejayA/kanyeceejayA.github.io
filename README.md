...
- `js/site.js` runs the theme switch, menu drawer, section rail and phone section pill, years strip, work group-by switch, project rows and their pop-up, the compact, summary and full views for jobs and papers, and hearts and comments on notes.
- `js/extras.js` adds the console greeting, the `akbr.*` console commands and the hidden extras. It loads `js/game.js` and `js/desk.js` only when someone finds them.
- `images/` holds the portrait, project screenshots, row thumbnails (`images/thumbs/`), desk prints (`images/desk/`), icons and the social card (`images/og-card.jpg`).
...
- **A note:** copy `notes/template.html`, set the title, date, description and the `data-heart` and `data-note` values in its reactions block, then add it to the list in `notes/index.html`.
- **A photo on the desk:** add a line to `PRINTS` at the top of `js/desk.js`.
- **Any `[data-modal="id"]` button** opens the `dialog` with that id.
